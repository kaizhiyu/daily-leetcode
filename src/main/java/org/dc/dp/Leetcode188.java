...
public class Leetcode188 {
    ...
    // 引申
    // 参考：
    // 1) https://www.bilibili.com/video/BV1ho4y1W7QK/?spm_id_from=333.788&vd_source=90ccec7f63fd55273154473f8d9d8ae0 ;
    // 2) https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iv/solutions/2201488/shi-pin-jiao-ni-yi-bu-bu-si-kao-dong-tai-kksg/

    // 如果改成「恰好」完成 k 笔交易要怎么做？
    // 递归到 i<0 时，只有 j=0 才是合法的，j>0 是不合法的。具体实现就是：f[0][1][0] = 0 , 其余为-∞（这么设置的原因：状态转移的时候都是使用max取值，一旦值正常的时候，则必然在max后取到正常值），
    // 注意前面加了个状态，f[0][1]才是恰好完成0次的状态

    // 如果改成「至少」完成 k 笔交易要怎么做？
    // 递归到「至少 0 次」时，它等价于「交易次数没有限制」，那么这个状态的计算方式和 122. 买卖股票的最佳时机 II 是一样的。
    // f[i][-1][.] 等价于 f[i][0][.]
    // 所以每个f[i]的前面不需要添加状态
    // 至少0次等价于可以无限次交易，所以f[i][0][.]就是无限次交易下的最大利润，转移方程也一样
    // f[0][0][0] = 0 , 其余 = -∞
