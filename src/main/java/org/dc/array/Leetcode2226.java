...
public class Leetcode2226 {
    public int maximumCandies(int[] candies, long k) {
        ...
        long right = sum / k, left = 0;

        while (left <= right) {
            long mid = (left + right) / 2;
            if (cnt(candies, k, mid) >= k) {
                left = mid + 1;
            }
            else {
                right = mid - 1;
    ...
    // 每个孩子分candiesNums个糖，能分多少个孩子
    private long cnt(int[] candies, long k, long candiesNums) {
        if (candiesNums == 0) {
            return k + 1; // 避免除数是0, candiesNums == 0时一定够分，直接返回k + 1
        }
